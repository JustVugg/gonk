...
var startCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if daemon {
			fmt.Fprintln(os.Stderr, "daemon mode is not built in; run GONK under systemd, Docker, or your process manager")
			os.Exit(2)
		} else {
...
var routesAddCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		stripPath, _ := cmd.Flags().GetBool("strip-path")

		if err := addRoute(configPath, name, path, upstream, methods, authType, stripPath); err != nil {
			fmt.Fprintln(os.Stderr, err)
			os.Exit(1)
		}
	},
...
func init() {
	rootCmd.PersistentFlags().StringVar(&gonkURL, "url", defaultGonkURL, "GONK admin URL")
	...
	routesAddCmd.Flags().StringP("config", "c", "gonk.yaml", "Configuration file path")
	routesAddCmd.Flags().String("name", "", "Route name")
	routesAddCmd.Flags().String("path", "", "Route path, for example /api/*")
	routesAddCmd.Flags().String("upstream", "", "Upstream URL")
	routesAddCmd.Flags().StringSlice("methods", []string{"GET"}, "Allowed HTTP methods")
	routesAddCmd.Flags().String("auth", "none", "Auth type (none, jwt, api_key, mtls)")
	routesAddCmd.Flags().Bool("strip-path", false, "Strip the route prefix before proxying")
	...
	authAPIKeyListCmd.Flags().StringP("config", "c", "gonk.yaml", "Configuration file path")
	...
	logsTailCmd.Flags().String("file", "", "Log file path, or set GONK_LOG_FILE")
